...
interface Schedule {
  ...
  type: 'pipe' | 'install' | 'as' | 'other';
}

const TYPE_LABEL = { pipe:'선배관', install:'기계설치', as:'AS', other:'기타' };
const TYPE_COLOR = { pipe:'bg-blue-400', install:'bg-green-400', as:'bg-orange-400', other:'bg-gray-400' };
const TYPE_TEXT  = { pipe:'bg-blue-100 text-blue-700', install:'bg-green-100 text-green-700', as:'bg-orange-100 text-orange-700', other:'bg-gray-100 text-gray-700' };
...
export default function DashboardPage() {
  ...
  const [form, setForm] = useState({ title:'', type:'other' as Schedule['type'] });

  const calStart = startOfWeek(startOfMonth(cur), { weekStartsOn: 0 });
  const calEnd   = endOfWeek(endOfMonth(cur),   { weekStartsOn: 0 });
  const rows: Date[][] = [];
  let d = calStart;
  while (d <= calEnd) {
    const row: Date[] = [];
  ...
  const weekDays = Array.from({ length: 7 }, (_, i) =>
    addDays(startOfWeek(today, { weekStartsOn: 0 }), i)
  );

  const forDate = (date: string) => schedules.filter(s => s.date === date);
  const todaySchedules = forDate(todayStr);
      ...
      <div className="flex gap-4">
        ...
        <div className="flex-1 bg-white rounded-xl border border-gray-200 p-4">
          <div className="flex items-center justify-between mb-4">
            <button onClick={() => setCur(subMonths(cur, 1))} className="p-2 rounded hover:bg-gray-100 text-gray-600">◀</button>
            <h2 className="text-lg font-bold text-gray-800">{format(cur, 'yyyy년 M월', { locale: ko })}</h2>
            <button onClick={() => setCur(addMonths(cur, 1))} className="p-2 rounded hover:bg-gray-100 text-gray-600">▶</button>
          </div>
          <div className="grid grid-cols-7 mb-1">
            {['일','월','화','수','목','금','토'].map((day, i) => (
              <div key={day} className={`text-center text-xs font-semibold py-1 ${i===0?'text-red-500':i===6?'text-blue-500':'text-gray-500'}`}>{day}</div>
            ))}
          </div>
            ...
            {rows.map((row, ri) => (
              <div key={ri} className="grid grid-cols-7">
                {row.map((date, di) => {
                  ...
                  const ds = forDate(dateStr);
                  const inMonth = isSameMonth(date, cur);
                  return (
                    <div key={di} onClick={() => { setSelDate(dateStr); setShowForm(true); }}
                      className={`min-h-[80px] p-1.5 border-b border-r border-gray-100 cursor-pointer transition-colors
                        ...
                        ${selDate === dateStr ? 'ring-2 ring-inset ring-blue-400' : ''}`}>
                      <span className={`inline-flex items-center justify-center w-6 h-6 rounded-full text-xs font-medium mb-1
                        ...
                        ${!inMonth ? 'text-gray-300' : di===0 ? 'text-red-500' : di===6 ? 'text-blue-500' : 'text-gray-700'}`}>
                        {format(date, 'd')}
                      </span>
                      <div className="space-y-0.5">
                        {ds.slice(0, 2).map(s => (
                          <div key={s.id} className={`text-white text-[10px] px-1 rounded truncate ${TYPE_COLOR[s.type]}`}>{s.title}</div>
                        ))}
        ...
        <div className="w-64 flex flex-col gap-4">
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-xs text-gray-400">{format(today, 'M월 d일 (E)', { locale: ko })}</span>
            </div>
              ...
              <ul className="space-y-2">
                {todaySchedules.map(s => (
                  <li key={s.id} className="flex items-center justify-between gap-2">
                    ...
                    <button onClick={() => deleteSchedule(s.id)} className="text-red-400 hover:text-red-600 text-xs shrink-0">✕</button>
                  </li>
                ))}
              </ul>
            )}
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-4">
            ...
            <div className="space-y-1.5">
              {weekDays.map(day => {
                const ds = forDate(format(day, 'yyyy-MM-dd'));
                return (
                  <div key={day.toISOString()} className="flex items-start gap-2">
                    <div className={`w-8 text-center shrink-0 ${isToday(day) ? 'font-bold text-blue-600' : 'text-gray-500'}`}>
                      <div className="text-[10px]">{format(day, 'E', { locale: ko })}</div>
                      <div className="text-xs">{format(day, 'd')}</div>
                    </div>
                    <div className="flex-1 min-w-0 pt-0.5">
                      {ds.length === 0 ? <div className="h-4 border-b border-gray-100" /> : (
                        <div className="space-y-0.5">
                          {ds.map(s => <div key={s.id} className={`text-white text-[10px] px-1.5 py-0.5 rounded truncate ${TYPE_COLOR[s.type]}`}>{s.title}</div>)}
                        </div>
                      )}
                    </div>
                  </div>
          ...
          <div className="bg-white rounded-xl p-6 w-80 shadow-xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <select value={form.type} onChange={e => setForm({...form, type: e.target.value as Schedule['type']})}
